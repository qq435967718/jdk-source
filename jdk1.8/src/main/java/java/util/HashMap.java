...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    /**
     * The default initial capacity - MUST be a power of two.默认的初始容量（容量为HashMap中槽的数目）是16，且实际容量必须是2的整数次幂。
     */
     ...
     * MUST be a power of two <= 1<<30.最大容量（必须是2的幂且小于2的30次方，传入容量过大将被这个值替换）
     */
    static final int MAXIMUM_CAPACITY = 1 << 30;

    /**
     * The load factor used when none specified in constructor.默认装填因子0.75，如果当前键值对个数 >= HashMap最大容量*装填因子，进行rehash操作
     */
     ...
     * shrinkage. JDK1.8 新加，Entry链表最大长度，当桶中节点数目大于该长度时，将链表转成红黑树存储；
     */
     ...
     * most 6 to mesh with shrinkage detection under removal.JDK1.8 新加，当桶中节点数小于该长度，将红黑树转为链表存储；
     */
     ...
     * between resizing and treeification thresholds.桶可能被转化为树形结构的最小容量。当哈希表的大小超过这个阈值，才会把链式结构转化成树型结构，否则仅采取扩容来尝试减少冲突。
     ...
     * TreeNode subclass, and in LinkedHashMap for its Entry subclass.) JDK1.6用Entry描述键值对，JDK1.8中用Node代替Entry
     */
     ...
     * Comparable<C>", else null.如果参数x实现了Comparable接口，返回参数x的类名，否则返回null
     */
     ...
     * class), else 0.如果x的类型为kc，则返回k.compareTo(x)，否则返回0
     */
    ...
    /**
     * Returns a power of two size for the given target capacity.结果为>=cap的最小2的自然数幂
     */
     ...
     * bootstrapping mechanics that are currently not needed.)哈希桶数组，分配的时候，table的长度总是2的幂
     */
    transient Node<K,V>[] table;

    /**
     * Holds cached entrySet(). Note that AbstractMap fields are used
     * for keySet() and values().HashMap将数据转换成set的另一种存储形式，这个变量主要用于迭代功能
     */
    transient Set<Map.Entry<K,V>> entrySet;

    /**
     * The number of key-value mappings contained in this map.实际存储的数量，则HashMap的size()方法，实际返回的就是这个值，isEmpty()也是判断该值是否为0
     */
    transient int size;
     ...
     * the HashMap fail-fast.  (See ConcurrentModificationException).hashmap结构被改变的次数，fail-fast机制,用于iterator遍历，如果其他方法改变modCount，会被iterator感知，从而跑出异常
     */
     ...
     * (16) and the default load factor (0.75).使用指定的初始化容量（16）和默认加载因子DEFAULT_LOAD_FACTOR（0.75）构造一个空HashMap
     */
    public HashMap() {
    ...
    final void putMapEntries(Map<? extends K, ? extends V> m, boolean evict) {
        int s = m.size();
        if (s > 0) {//如果参数map不为空
            if (table == null) { // pre-size// 判断table是否已经初始化
                float ft = ((float)s / loadFactor) + 1.0F;// 未初始化，s为m的实际元素个数
                int t = ((ft < (float)MAXIMUM_CAPACITY) ?
                         (int)ft : MAXIMUM_CAPACITY);
                if (t > threshold)// 计算得到的t大于阈值，则初始化阈值
                    threshold = tableSizeFor(t);//根据容量初始化临界值
            } // 已初始化，并且m元素个数大于阈值，进行扩容处理
            else if (s > threshold)
                resize();//扩容处理
            for (Map.Entry<? extends K, ? extends V> e : m.entrySet()) { // 将m中的所有元素添加至HashMap中
                K key = e.getKey();
                V value = e.getValue();
    ...
    final Node<K,V> getNode(int hash, Object key) {
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&//如果哈希表不为空，而且key对应的桶上不为空
            (first = tab[(n - 1) & hash]) != null) {
            if (first.hash == hash && // always check first node //如果桶中的第一个节点就和指定参数hash和key匹配上了
                ((k = first.key) == key || (key != null && key.equals(k))))
                return first;//返回桶中的第一个节点
            if ((e = first.next) != null) {//如果桶中的第一个节点没有匹配上，而且有后续节点
                if (first instanceof TreeNode)//如果当前的桶采用红黑树，则调用红黑树的get方法去获取节点
                    return ((TreeNode<K,V>)first).getTreeNode(hash, key);
                do {//如果当前的桶不采用红黑树，即桶中节点结构为链式结构
                    if (e.hash == hash &&//遍历链表，直到key匹配
                        ((k = e.key) == key || (key != null && key.equals(k))))
                        return e;
                } while ((e = e.next) != null);
            }
        }
        return null; //如果哈希表为空，或者没有找到节点，返回null
    }
    ...
    final Node<K,V>[] resize() {
        ...
        else if (oldThr > 0) // initial capacity was placed in threshold//如果旧容量 <= 0，而且旧临界值 > 0
            //数组的新容量设置为老数组扩容的临界值
            newCap = oldThr;
        else {// zero initial threshold signifies using defaults//如果旧容量 <= 0，且旧临界值 <= 0，新容量扩充为默认初始化容量，新临界值为DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY
        ...
        if (oldTab != null) {
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    ...
                    else { // preserve order//如果旧桶中的结构为链表,链表重排，jdk1.8做的一系列优化
     ...
     * table is too small, in which case resizes instead. 将链表转化为红黑树
     */
     ...
     * The map will be empty after this call returns.删除map中所有的键值对
     */
    public void clear() {
        Node<K,V>[] tab;
        modCount++;
    ...
    // Overrides of JDK8 Map extension methodsJDK8重写的方法
    /**
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        if (mappings < 0)
            ...
            // Read the keys and values, and put the mappings in the HashMap// 读出key和value，并组成键值对插入hashMap中
    ...
    static class HashMapSpliterator<K,V> {
        ...
        Node<K,V> current;          // current node//记录当前的节点
        int index;                  // current index, modified on advance/split//当前节点的下标
        int fence;                  // one past last index//堆大小
        int est;                    // size estimate//估计大小
    ...
    // Create a regular (non-tree) node创建一个链表结点
    Node<K,V> newNode(int hash, K key, V value, Node<K,V> next) {
        return new Node<>(hash, key, value, next);
    }

    // For conversion from TreeNodes to plain nodes替换一个链表节点
    Node<K,V> replacementNode(Node<K,V> p, Node<K,V> next) {
        return new Node<>(p.hash, p.key, p.value, next);
    }

    // Create a tree bin node创建一个红黑树节点
    TreeNode<K,V> newTreeNode(int hash, K key, V value, Node<K,V> next) {
        return new TreeNode<>(hash, key, value, next);
    }

    // For treeifyBin替换一个红黑树节点
    ...
    // Called only from writeObject, to ensure compatible ordering.写入hashMap键值对到ObjectOutputStream中
    ...
    static final class TreeNode<K,V> extends LinkedHashMap.Entry<K,V> {
        ...
        /**
         * Ensures that the given root is the first node of its bin.确保root是桶中的第一个元素 ，将root移到中中的第一个
         */
         ...
         * comparing keys.查找hash为h，key为k的节点
         */
        ...
        /**
         * Calls find for root node.获取树节点，通过根节点查找
         */
         ...
         * necessary simplifies testing a bit.比较2个对象的大小
         */
        ...
        /* ------------------------------------------------------------ */
        // Red-black tree methods, all adapted from CLR红黑树方法，都是从CLR中修改的
        /**
         * 左旋转
         */
        ...
        /**
         * Recursive invariant check检测是否符合红黑树
         */
