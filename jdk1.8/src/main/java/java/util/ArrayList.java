        ...
        implements List<E>, RandomAccess, Cloneable, java.io.Serializable
{
     ...
     * @throws IllegalArgumentException if the specified initial capacity 如果指定容量为负
     *         is negative
     */
    public ArrayList(int initialCapacity) {
        ...
        } else {//如果用户初始化大小小于0抛异常，否则新建一个用户初始值大小的object数组。
    ...
    /**
     * Constructs an empty list with an initial capacity of ten.不带参数的构造方法
     */
    public ArrayList() {
     ...
     * iterator.
     * 带参数Collection的构造方法
     * @param c the collection whose elements are to be placed into this list 其元素将被放入此列表中的集合
     * @throws NullPointerException if the specified collection is null 如果指定的集合是空的
     */
    public ArrayList(Collection<? extends E> c) {
        elementData = c.toArray();
        if ((size = elementData.length) != 0) {
            // c.toArray might (incorrectly) not return Object[] (see 6260652) // c.toarray可能（错误地）不返回对象[]（见JAVA BUG编号6260652）
    ...
    public void ensureCapacity(int minCapacity) {
        ...
        int minExpand = (elementData != DEFAULTCAPACITY_EMPTY_ELEMENTDATA)
            ...
            : DEFAULT_CAPACITY;//调用ArrayList(10) 默认初始化一个大小为10的object数组。
    ...
    private void grow(int minCapacity) {
        ...
        // 扩容至原来的1.5倍
        int newCapacity = oldCapacity + (oldCapacity >> 1);//带符号右移，  10>>1=5
        // 再判断一下新数组的容量够不够，够了就直接使用这个长度创建新数组，
        // 不够就将数组长度设置为需要的长度
        if (newCapacity - minCapacity < 0)//与默认增长对比，取更大的数
     ...
     * 返回ArrayList的大小
     * @return the number of elements in this list ArrayList中的元素数量
     */
    public int size() {
        return size;
    }
     ...
     * 返回是否为空
     * @return <tt>true</tt> if this list contains no elements 如果ArrayList中无元素
     */
     ...
     * 是否包含一个数 返回bool
     * @param o element whose presence in this list is to be tested 检测o是否为ArrayList中元素
     * @return <tt>true</tt> if this list contains the specified element 如果ArrayList中包含o元素
     */
     ...
     * or -1 if there is no such index. 返回一个值在数组首次出现的位置，会根据是否为null使用不同方式判断。不存在就返回-1。时间复杂度为O(N)
     */
    public int indexOf(Object o) {
        if (o == null) {//如果为空，则查找list为空的下标
     ...
     * <tt>(o==null&nbsp;?&nbsp;get(i)==null&nbsp;:&nbsp;o.equals(get(i)))</tt>,
     * or -1 if there is no such index. 返回一个值在数组最后一次出现的位置，会根据是否为null使用不同方式判断。不存在就返回-1。时间复杂度为O(N)
     */
     ...
     * @return a clone of this <tt>ArrayList</tt> instance ArrayList副本
     */
    public Object clone() {//clone1个数组
        try {
     ...
     * @return an array containing all of the elements in this list in 一个数组包含所有列表中的元素, 且顺序正确
     *         proper sequence
     */
     ...
     *          same runtime type is allocated for this purpose.    如果它的长度大的话，列表元素将存储在这个数组中; 否则，将为此分配一个相同运行时类型的新数组。
     * @return an array containing the elements of the list 一个包含ArrayList元素的数组
     * @throws ArrayStoreException if the runtime type of the specified array
     *         is not a supertype of the runtime type of every element in 将与数组类型不兼容的值赋值给数组元素时抛出的异常
     *         this list
     * @throws NullPointerException if the specified array is null  数组为空
     */
    ...
    public <T> T[] toArray(T[] a) {
        ...
        System.arraycopy(elementData, 0, a, 0, size);//src表示源数组，srcPos表示源数组要复制的起始位置，desc表示目标数组，length表示要复制的长度。这是浅复制
        if (a.length > size)
            a[size] = null;
        return a;
    }
     ...
     */
    @SuppressWarnings("unchecked")
    E elementData(int index) {//获取elementData数组的index处的值
     ...
     * @return the element at the specified position in this list ArrayList中指定位置的元素
     ...
     * 添加一个值，首先会确保容量
     * @param e element to be appended to this list 要添加到此列表中的元素
     ...
     * @param index index at which the specified element is to be inserted 指定元素将被插入的索引
     ...
     * @return the element that was removed from the list 从ArrayList中删除的元素
     * @throws IndexOutOfBoundsException {@inheritDoc}
     */
    public E remove(int index) {
        ...
        elementData[--size] = null; // clear to let GC do its work//便于垃圾回收器回收
     ...
     * @param o element to be removed from this list, if present 要从该列表中删除的元素（如果存在）
     * @return <tt>true</tt> if this list contained the specified element 如果这个列表包含指定的元素
     */
     ...
     * @param c collection containing elements to be added to this list 包含要添加到此列表中的元素的集合
     * @return <tt>true</tt> if this list changed as a result of the call 如果该列表因添加而改变
     * @throws NullPointerException if the specified collection is null 如果指定的集合是空的
     */
     ...
     */
    private void rangeCheck(int index) {//检查下标是否超出size
        if (index >= size)
     ...
     */
    private void rangeCheckForAdd(int index) {//下标检查
